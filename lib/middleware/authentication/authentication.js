...
module.exports = function (req, res, next) {
  ...
  if (env === 'production' && useAuth === 'true') {
    ...
    const messageParts = ['User', user, {username, password}]

    if (!user || user.name !== username || user.pass !== password) {
      res.set('WWW-Authenticate', 'Basic realm=Authorization Required')
      messageParts.push('Auth required')
      messageParts.push(!user)
      if (user) {
        messageParts.push(user.name)
        messageParts.push(username)
        messageParts.push(user.name !== username)
        messageParts.push(user.pass)
        messageParts.push(password)
        messageParts.push(user.pass !== password)
        messageParts.push(req.headers ? req.headers.authorization : req.get('authorization'))
      }
      console.log(`[${messageParts.map(JSON.stringify.bind(JSON)).join('] [')}]`)
      return res.sendStatus(401)
    }
    messageParts.push('Authenticated')
    console.log(`[${messageParts.map(JSON.stringify.bind(JSON)).join('] [')}]`)
  }
  next()
}
